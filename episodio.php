...
$epique="SELECT * FROM `episodios` WHERE `idEpisodios` = ".$episodio;
$manepi=mysql_query($epique);
$arrayepi=mysql_fetch_row($manepi);
$titulo=$arrayepi[4];
$sinopsis=$arrayepi[1];
//Inicio de las cadenas de episodios y descargas
$quelinks="SELECT * FROM `links` WHERE `Episodios_idEpisodios` = ".$episodio;
$manlinks=mysql_query($quelinks);
if($manlinks!=false){
   $cadenatabs=null;
   $cadenalinks="<h4>Si algún archivo les pide contraseña la contraseña es <b>TheFlashUniverse</b></h4></br>";
   $cadenaetiquetas=null;
   $id=activo($episodio);
   while($arrlink=mysql_fetch_row($manlinks)){
    $etiqueta=$arrlink[1];
    $link=$arrlink[2];
    $tipo=$arrlink[3];
    $activo=$arrlink[4];
    $server=buscaserver($link);
    if($activo==1){
    	$id=1;
    }
    if($server=="openload"){
      ...
      if($tipo==1){
      $cadenatabs=$cadenatabs.agregavideo($link, $etiqueta, $id);
      $cadenaetiquetas=$cadenaetiquetas.agregaetiqueta($etiqueta, $id);
      }
      if($tipo==2){
      $cadenalinks=$cadenalinks.agregadescarga($link, $etiqueta);
    ...
    if($server=="otro"){
       $cadenalinks=$cadenalinks.agregaadfly($link, $etiqueta);
       }
    if($server=="vimeo"){
        $cadenatabs=$cadenatabs.agregavimeo($link, $etiqueta, $id);
        $cadenaetiquetas=$cadenaetiquetas.agregaetiqueta($etiqueta, $id);
   }
   $id++;
   }
...
<html lang="en">
<head>
<title><?php echo("The Flash universe -- ".$titulo); ?></title>
<!-- for-mobile-apps -->
				...
				<div class="col-sm-8 single-left">
					<div class="song">
						<div class="song-info">
							...
							<h4><?php echo($sinopsis);?></h4>
					</div>
						<div class="video-grid-single-page-agileits">
							<?php echo($cuerpodelpost);
						...
						<div class="share">
							...
							<ul>
								...
								<li class="news-twitter">
									<a href="https://twitter.com/theflashlatam" class="twitter-follow-button" data-show-count="false">Siguenos en twitter</a><script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>
								</li>
								<li>
									<a href="https://twitter.com/intent/tweet?screen_name=theflashlat" class="twitter-mention-button" data-show-count="false">Mandanos un tweet</a><script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>
								</li>
								<li>
